...
# Cache cho Khoa và NganhDaoTao (dùng trong import)
_khoa_cache = None
_nganh_cache = None

def get_khoa_cache():
    global _khoa_cache
    if _khoa_cache is None:
        _khoa_cache = {k.ma_khoa: k for k in Khoa.objects.all() if k.ma_khoa}
        # Cache theo tên (không dấu) để tìm kiếm linh hoạt
        for k in Khoa.objects.all():
            _khoa_cache[k.ten_khoa.strip().lower()] = k
    return _khoa_cache

def get_nganh_cache():
    global _nganh_cache
    if _nganh_cache is None:
        _nganh_cache = {}
        for n in NganhDaoTao.objects.select_related('khoa').all():
            key = (n.khoa_id, n.ten_nganh.strip().lower())
            _nganh_cache[key] = n
            # Lưu theo tên không phân biệt khoa (ưu tiên khoa trùng)
            if n.ten_nganh.strip().lower() not in _nganh_cache:
                _nganh_cache[n.ten_nganh.strip().lower()] = n
    return _nganh_cache

def reset_import_cache():
    """Reset cache để lần import sau lấy dữ liệu mới."""
    global _khoa_cache, _nganh_cache
    _khoa_cache = None
    _nganh_cache = None
...
def ensure_student(mssv, ho_ten='', lop='', khoa_name='', email='', phone='', ten_nganh='', ma_lop=''):
    """Phiên bản tối ưu hóa với cache để import nhanh."""
    mssv = extract_mssv(mssv)
    if not mssv:
        return None

    ho_ten = ho_ten or f'SV_{mssv}'
    khoa_cache = get_khoa_cache()
    nganh_cache = get_nganh_cache()

    # --- Xác định Khoa ---
    khoa_obj = None
    if khoa_name:
        key = khoa_name.strip().lower()
        khoa_obj = khoa_cache.get(key)
        if not khoa_obj:
            khoa_obj, _ = Khoa.objects.get_or_create(ten_khoa=khoa_name)
            khoa_cache[key] = khoa_obj
    else:
        ma_khoa = mssv[3:6] if len(mssv) >= 6 else ''
        if ma_khoa:
            khoa_obj = khoa_cache.get(ma_khoa)
        if not khoa_obj:
    ...
    nganh_obj = None
    if ten_nganh:
        key = (khoa_obj.id, ten_nganh.strip().lower()) if khoa_obj else ten_nganh.strip().lower()
        nganh_obj = nganh_cache.get(key)
        if not nganh_obj and khoa_obj:
            loai_nganh = detect_loai_nganh(ten_nganh)
            nganh_obj, _ = NganhDaoTao.objects.get_or_create(
                khoa=khoa_obj,
                ten_nganh=ten_nganh.strip(),
                defaults={'loai_nganh': loai_nganh, 'is_active': True}
            )
            nganh_cache[key] = nganh_obj

    # --- Tính các trường khác ---
    ...
    if not user_created:
        if user.first_name != ho_ten or (email and user.email != email):
    ...
    if not created:
        ...
        if sv.user_id != user.id:
            fields_to_update['user'] = user
            changed = True
        if sv.ho_ten != ho_ten:
            fields_to_update['ho_ten'] = ho_ten
            changed = True
        if lop and sv.lop != lop:
            fields_to_update['lop'] = lop
            changed = True
        if khoa_obj and sv.khoa_id != khoa_obj.id:
            fields_to_update['khoa'] = khoa_obj
            changed = True
        if email and sv.email_truong != email:
            fields_to_update['email_truong'] = email
            changed = True
        if phone and sv.so_dien_thoai != phone:
            fields_to_update['so_dien_thoai'] = phone
            changed = True
        if nganh_obj and sv.nganh_dao_tao_id != nganh_obj.id:
            fields_to_update['nganh_dao_tao'] = nganh_obj
            changed = True
        if khoa_tuyen_sinh and sv.khoa_tuyen_sinh != khoa_tuyen_sinh:
            fields_to_update['khoa_tuyen_sinh'] = khoa_tuyen_sinh
            changed = True
        if nam_nhap_hoc and sv.nam_nhap_hoc != nam_nhap_hoc:
            fields_to_update['nam_nhap_hoc'] = nam_nhap_hoc
            changed = True
        if nam_du_kien_tot_nghiep and sv.nam_du_kien_tot_nghiep != nam_du_kien_tot_nghiep:
            fields_to_update['nam_du_kien_tot_nghiep'] = nam_du_kien_tot_nghiep
            changed = True
        if chuong_trinh and sv.chuong_trinh_dao_tao != chuong_trinh:
            fields_to_update['chuong_trinh_dao_tao'] = chuong_trinh
            changed = True
        if changed:
...
from .models import Khoa, LichSuThi, ChungChi
...
def admin_mofi_dashboard(request):
    ...
    def count_status(khoa_sv_qs, mon_thi_code, status):
        """
        status: 'chua_co' -> chưa có lịch sử thi
                'chua_dat' -> có điểm thi nhưng chưa đạt (không có chứng chỉ)
                'da_dat'   -> có điểm thi đạt hoặc có chứng chỉ hợp lệ
        """
        # Xác định loại chứng chỉ tương ứng
        if mon_thi_code in ['TA_DAU_VAO', 'CDR_NGOAI_NGU']:
            loai_cc = 'NGOAI_NGU'
        else:  # CDR_TIN_HOC
            loai_cc = 'TIN_HOC'

        if status == 'chua_co':
            # Chưa có bất kỳ bản ghi điểm thi nào
            ...
            # Có điểm thi nhưng chưa đạt (không có điểm đạt, không có chứng chỉ đạt)
            return khoa_sv_qs.filter(
                Exists(LichSuThi.objects.filter(
                    sinh_vien=OuterRef('pk'),
                    mon_thi=mon_thi_code
                ...
                ~Exists(ChungChi.objects.filter(
                    sinh_vien=OuterRef('pk'),
                    trang_thai='DAT',
            ...
            # Đạt: có điểm thi đạt hoặc có chứng chỉ đạt
            return khoa_sv_qs.filter(
                Q(Exists(LichSuThi.objects.filter(
                    sinh_vien=OuterRef('pk'),
                    mon_thi=mon_thi_code,
                    ket_qua_dat=True
                ))) |
                Q(Exists(ChungChi.objects.filter(
                    sinh_vien=OuterRef('pk'),
                    trang_thai='DAT',
                    danh_muc__loai=loai_cc
                )))
    ...
    for khoa in Khoa.objects.all().order_by('ten_khoa'):
        ...
        thong_ke_khoa.append({
            ...
            'tong_sv': tong,
            # Tiếng Anh đầu vào
            'tdnn_chua_co': count_status(sv_qs, 'TA_DAU_VAO', 'chua_co'),
            'tdnn_chua_dat': count_status(sv_qs, 'TA_DAU_VAO', 'chua_dat'),
            'tdnn_da_dat': count_status(sv_qs, 'TA_DAU_VAO', 'da_dat'),
            # CĐR Ngoại ngữ
            'cdr_nn_chua_co': count_status(sv_qs, 'CDR_NGOAI_NGU', 'chua_co'),
            'cdr_nn_chua_dat': count_status(sv_qs, 'CDR_NGOAI_NGU', 'chua_dat'),
            'cdr_nn_da_dat': count_status(sv_qs, 'CDR_NGOAI_NGU', 'da_dat'),
            # CĐR Tin học
            'cdr_th_chua_co': count_status(sv_qs, 'CDR_TIN_HOC', 'chua_co'),
            'cdr_th_chua_dat': count_status(sv_qs, 'CDR_TIN_HOC', 'chua_dat'),
            'cdr_th_da_dat': count_status(sv_qs, 'CDR_TIN_HOC', 'da_dat'),
        })

    # ---- CHUẨN BỊ DỮ LIỆU CHO BIỂU ĐỒ ----
    labels = [khoa['ten_khoa'] for khoa in thong_ke_khoa]
    chart_data = {
        'labels': labels,
        'tdnn': {
            'chua_co': [khoa['tdnn_chua_co'] for khoa in thong_ke_khoa],
            'chua_dat': [khoa['tdnn_chua_dat'] for khoa in thong_ke_khoa],
            'da_dat': [khoa['tdnn_da_dat'] for khoa in thong_ke_khoa],
        },
        'cdr_nn': {
            'chua_co': [khoa['cdr_nn_chua_co'] for khoa in thong_ke_khoa],
            'chua_dat': [khoa['cdr_nn_chua_dat'] for khoa in thong_ke_khoa],
            'da_dat': [khoa['cdr_nn_da_dat'] for khoa in thong_ke_khoa],
        },
        'cdr_th': {
            'chua_co': [khoa['cdr_th_chua_co'] for khoa in thong_ke_khoa],
            'chua_dat': [khoa['cdr_th_chua_dat'] for khoa in thong_ke_khoa],
            'da_dat': [khoa['cdr_th_da_dat'] for khoa in thong_ke_khoa],
        }
    }
...
def import_sinh_vien(request):
    ...
    if request.method == 'POST':
        ...
        # Bọc trong transaction để tối ưu hiệu năng
        ...
        try:
            ...
            with transaction.atomic():
                for _, row in df.iterrows():
                    ...
                    count += 1
            # Reset cache vì có thể đã tạo mới Khoa/Ngành
            reset_import_cache()
