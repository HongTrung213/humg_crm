...
class LichSuThiInline(admin.TabularInline):
    ...
    # Đã gộp và hiển thị đầy đủ điểm thành phần
    fields = ('mon_thi', 'dot_thi', 'diem_thanh_phan_1', 'diem_thanh_phan_2', 'diem_thi', 'ket_qua_dat')
    readonly_fields = ('ket_qua_dat',) # Máy tự tính
...
class LopBoiDuongAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Nạp sinh viên vào lớp', {
            ...
            'description': 'Tải file Excel (có cột ghi chữ MSSV) hoặc tự chọn tay sinh viên ở dưới.'
        }),
    )
...
class DangKyLopAdmin(admin.ModelAdmin):
    # Hiển thị thêm cột Ảnh biên lai và Trạng thái có màu
    ...
    # Hàm hiển thị ảnh thumbnail biên lai
    def hien_thi_bien_lai(self, obj):
        if obj.file_minh_chung:
            return format_html('<a href="{}" target="_blank"><img src="{}" style="height: 50px; border-radius: 5px; border: 1px solid #ddd;"/></a>', 
                               obj.file_minh_chung.url, obj.file_minh_chung.url)
        return "Trống"
    ...
    def trang_thai_mau(self, obj):
        ...
        return format_html('<b style="color: {};">{}</b>', 
                           colors.get(obj.trang_thai, 'black'), obj.get_trang_thai_display())
    ...
    def duyet_dang_ky(self, request, queryset):
        ...
        for dang_ky in queryset:
            if dang_ky.lop_hoc.sinh_vien.count() < dang_ky.lop_hoc.si_so_toi_da:
                ...
                dang_ky.save() # Tự động kích hoạt hàm save() trong models để add SV vào lớp
                count += 1
